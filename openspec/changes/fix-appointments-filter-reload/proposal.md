...
4. **Debounce on dropdowns**: Add a debounce mechanism to `onFilterChange()` similar to the existing `onSearch()` debounce, to prevent rapid API calls when switching filters quickly.
