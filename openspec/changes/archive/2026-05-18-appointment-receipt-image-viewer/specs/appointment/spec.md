# Delta Specs: Visor de Imágenes para Comprobantes

## Domain: Appointment Receipt Viewer
...
### Requirement: REQ-MOD-001 — Reemplazar enlaces `<a>` por elementos clickeables
**SHALL** Los enlaces `<a>` con `target="_blank"` en `appointments.component.html` y `employee-detail-dialog.component.html` serán reemplazados por elementos con handlers de clic que abran el visor integrado.

#### Scenario: Reemplazo de enlaces externos
- **Given** una cita con `receipt_url` y/o `payment_receipt_url`
- **When** el usuario hace clic en el icono de comprobante
- **Then** debe abrirse un `p-dialog` integrado con la imagen
- **And** no debe abrirse una nueva pestaña del navegador
